# -*- coding: utf-8 -*-
# https://github.com/selcukeskil/3-heuristic-algorithms-in-Python-for-Travelling-Salesman-Problem/tree/main/Genetic%20Algorithm
...
def main(mylist, generation_number, number_of_individuals, number_of_pairs_M, crossover_probability, mutation_probability):
    ...
    # create a DataFrame including only x and y
    d = {'x': xlist, 'y': ylist}
    df = pd.DataFrame(data=d)
    print(df)
    # fun.DrawRealMap(df, None, [(i%tmpList_len,(i+1)%tmpList_len) for i in range(tmpList_len)])
    ...
    for i in range(generation_number):
        ...
        # print(population)
        for solution, fitness in population.itertuples(index=False):
            # print("Cost: ",calculate_distance(cities , solution))
    ...
    cost=calculate_distance(cities , solution)
    print("Cost of that solution: ", cost )
    ...
    return (len(cities),round(comp_time,2), round(cost,2))
            
    
print("GA")
results=[]
for i in range(10,11): #i starts from 1!!!
    results.append(main(fun.generated_list_of_27[15],700, 100, 25, 0.7, 0.5)) #(generation number, number of individuals in a generation, Number of parent "pairs" to be selected in parent selection, crossover probability for a parent pair, mutation probability for a child solution)
    print(results[-1])
print (results)
